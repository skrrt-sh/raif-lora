...
_GRAD = [(137, 180, 250), (140, 170, 252), (160, 168, 250),
         (180, 170, 249), (203, 166, 247), (203, 166, 247)]
_LOGO = [
    "█████╗   █████╗  ██╗ ███████╗",
    "██╔═██╗ ██╔══██╗ ██║ ██╔════╝",
    "█████╔╝ ███████║ ██║ █████╗  ",
...
CASES = [
    {
        "key": "flat",
        "details": ('Capture this as a record. Set cuisine to "Italian". The '
                    'value of location is "New York". price_range should be '
                    '"$$". Set open_now to true. The value of rating is 4.6.'),
        "schema": ["cuisine:s", "location:s", "price_range:s",
                   "open_now:b", "rating:n"],
        "expected": {"cuisine": "Italian", "location": "New York",
                     "price_range": "$$", "open_now": True, "rating": 4.6},
        "budget": 256, "bug": "bloat",
    ...
    {
        "key": "typed",
        "details": ('Fill these fields. For subject.last_name use "Tanaka". '
                    'Assign "agent" to subject.role. subject.id should be 3129. '
                    'The value of session is "quiet-lantern-330".'),
        "schema": ["subject.last_name:s", "subject.role:s", "subject.id:n",
                   "session:s"],
        "expected": {"subject": {"last_name": "Tanaka", "role": "agent",
                                 "id": 3129},
                     "session": "quiet-lantern-330"},
        "budget": 256, "bug": "wrong",
    ...
    {
        "key": "table",
        "details": ("Build the rows from these line items.\n"
        ...
        "schema": ["properties[]:o", "properties[].code:n",
                   "properties[].completed:n", "properties[].extra:s"],
        "expected": {"properties": [
...
class Model:
    ...
    def __init__(self, lora: bool = False) -> None:
        ...
        if not (self.spec["mlx_dir"] / "adapters.safetensors").exists():
            sys.exit("Adapter not built. Run: "
    ...
    def stream(self, prompt: str, max_tokens: int, color: str = TEXT,
               cps: float = 120.0, cap_lines: int | None = None):
        ...
        prm = self.tok.apply_chat_template([{"role": "user", "content": prompt}],
                                           add_generation_prompt=True, tokenize=False)
        out, lines, capped, suppress = "", 0, False, False
        sys.stdout.write(INDENT + color); sys.stdout.flush()
        for resp in stream_generate(self.model, self.tok, prompt=prm,
                                    max_tokens=max_tokens, sampler=self.greedy):
            for ch in resp.text:
                if not suppress:
                    if ch == "\n":
                        lines += 1
...
def loading_screen() -> None:
    ...
    for row, art in zip(_GRAD, _LOGO):
        emit(f"{MARGIN}{BOLD}{fg(*row)}{art}{RESET}")
    emit()
    emit(f"{MARGIN}{GRAY}loading Qwen2.5-0.5B (MLX) — every answer below is "
...
def slide_fail(m: Model, num: str, case: dict) -> int:
    ...
    prompt = PROMPT_TMPL.format(details=case["details"], schema="\n".join(case["schema"]))
    ...
    if not ok:
        if truncated:
            emit(f"{INDENT}{RED}✗ no usable record — it bloated past the "
                 f"{ntok}-token budget and was cut off mid-row{RESET}")
        else:
            emit(f"{INDENT}{RED}✗ no usable record — it answered in prose, "
                 f"not structured data{RESET}")
    else:
        faithful = canon(val) == canon(expected)
        if faithful:
            emit(f"{INDENT}{YELLOW}✓ correct — {RED}but {ntok} tok, wrapped in "
                 f"prose and a code fence{RESET}")
            emit(f"{INDENT}{GRAY}↑ you'd still have to find and parse the JSON "
                 f"back out{RESET}")
        else:
...
def slide_success(m: Model, num: str, case: dict, base_tokens: int) -> None:
    ...
    if res.get("ok") and canon(res["value"]) == canon(expected):
        ...
        _sleep(0.3)
        win = (f"{rtok} tok vs base's {base_tokens}+ (cut off)"
